...
/**
 * Gestion d'un serveur : creation d'une connexion de type serveur pour attendre les connexions de clients
 * @author emds
 *
 */
public class ServeurSocket extends Thread {
	...
	/**
	 * objet de lien avec une autre classe qui implémente AsyncResponse pour transferer les réponses
	 */
	private AsyncResponse delegate=null; 
	
	/**
	 * Constructeur
	 * @param delegate instance de la classe vers laquelle il faut transferer les reponses
	 * @param port numero port d'écoute du serveur
	 */
	public ServeurSocket(AsyncResponse delegate, int port) {
		...
		try {
			...
			// le demarrage de l'ecoute ne peut se faire que si un objet delegate existe (pour récupérer la réponse)
			if(delegate != null) {
				this.start();		
			}
		} catch (IOException e) {
			// problème grave qui ne devrait pas se produire : arrêt du programme
			System.out.println("erreur grave creation socket serveur : "+e);
			System.exit(0);
		}
	}
	 ...
	 */
	public void run() {
		// objet qui va récuperer le socket du client qui s'est connecte
		Socket socket ;
